function varargout = SyncAD(varargin)
% SyncAD MATLAB code for SyncAD.fig
%      SyncAD, by itself, creates a new SyncAD or raises the existing
%      singleton*.
%
%      H = SyncAD returns the handle to a new SyncAD or the handle to
%      the existing singleton*.
%
%      SyncAD('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SyncAD.M with the given input arguments.
%
%      SyncAD('Property','Value',...) creates a new SyncAD or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SyncAD_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SyncAD_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SyncAD

% Last Modified by GUIDE v2.5 22-May-2017 20:25:51

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SyncAD_OpeningFcn, ...
                   'gui_OutputFcn',  @SyncAD_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SyncAD is made visible.
function SyncAD_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SyncAD (see VARARGIN)

% Choose default command line output for SyncAD
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SyncAD wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SyncAD_OutputFcn(hObject, eventdata, handles) 
...
[baddata_index]=MLE(untitled,Gtruth);
...
if value==1
SyncAD(plot(xData,baddata_detected,'bd'))
title('Anomaly Detection Plot')
ylabel('Anomaly Instance')
elseif value==2
    SyncAD(plot(xData,baddata_detected))
title('Raw Data Plot')
ylabel('Voltage')
elseif value==3
SyncAD(plot(xData,baddata_detected))
